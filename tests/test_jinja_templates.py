...
def test_jinja_vector_invite(sydent):
    substitutions = {
    ...
    with patch("sydent.util.emailutils.smtplib") as smtplib:
        sendEmail(sydent, templateFile, "test@test.com", substitutions)

    smtp = smtplib.SMTP.return_value
    email_contents = smtp.sendmail.call_args[0][2].decode("utf-8")

    # test url input is encoded
    assert urllib.parse.quote("mxc://example.org/s0meM3dia") in email_contents

    # test html input is escaped
    assert "Bob&#39;s Emporium of Messages" in email_contents

    # test safe values are not escaped
    assert "<Bob Smith>" in email_contents
...
def test_jinja_matrix_invite(sydent):
    substitutions = {
        "address": "foo@example.com",
        "medium": "email",
        "room_alias": "#somewhere:exmaple.org",
...
def test_jinja_matrix_verification(sydent):
    substitutions = {
        "address": "foo@example.com",
        "medium": "email",
        "to": "person@test.test",
    ...
    # test html input is escaped
    assert "&lt;&lt;token&gt;&gt;" in email_contents
...
def test_jinja_vector_verification(sydent):
    substitutions = {
        "address": "foo@example.com",
        "medium": "email",
        "to": "person@test.test",
    ...
    path = os.path.join(
        sydent.config.general.templates_path,
    ...
    assert email_contents == expected_text
