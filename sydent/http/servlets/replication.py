...
def _get_peer_certificate_cn(request: web.Request) -> str:
    """Extract the commonName from the peer's TLS certificate.

    :raises MatrixRestError: if no certificate is available.
    """
    ssl_object = (
        request.transport.get_extra_info("ssl_object") if request.transport else None
    )
    if ssl_object is None:
        raise MatrixRestError(403, "M_UNKNOWN_PEER", "No TLS certificate provided")

    peer_cert_der = ssl_object.getpeercert(binary_form=True)
    if peer_cert_der is None:
        raise MatrixRestError(403, "M_UNKNOWN_PEER", "No TLS certificate provided")

    x509 = load_certificate(FILETYPE_ASN1, peer_cert_der)
...
async def handle_replication_push_post(request: web.Request) -> web.Response:
    sydent = request.app["sydent"]
    hashing_store = HashingMetadataStore(sydent)

    peerCertCn = _get_peer_certificate_cn(request)

    peerStore = PeerStore(sydent)
    ...
    try:
        ...
        inJson = json_decoder.decode(body.decode("UTF-8"))
    except ValueError:
        logger.warning(
    ...
    globalAssocsStore = GlobalAssociationStore(sydent)
    ...
    for originId, sgAssoc in sg_assocs:
        try:
            ...
            if assocObj.mxid is not None:
                ...
                pepper = hashing_store.get_lookup_pepper()
